...
void Donut::donut(){
  ...
  //Checks neighbors by following the donut rules
      for (int i=0; i<length;i++){
        for(int j=0; j<width;j++){
          neighbors=0;
          if(i==0&&j==0){
            ...
            if(arr[length-1][width-1]=='X'){
              neighbors++;
            }
            if(arr[length-1][j]=='X'){
              neighbors++;
            }
            if(arr[length-1][j+1]=='X'){
              neighbors++;
            }
      ...
      //generation has ended and will start again
    }
    if(write==true){
